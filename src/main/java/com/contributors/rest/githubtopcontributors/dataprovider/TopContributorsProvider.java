package com.contributors.rest.githubtopcontributors.dataprovider;
...
@Slf4j
@Repository
public class TopContributorsProvider {
	...
	@Autowired
	public TopContributorsProvider(RestTemplateBuilder builder) {
	...
	public TopContributorsList findTopContributors(String place, Integer count) {
	    ...
	    TopContributorsList topContributors = new TopContributorsList(allContributors, new Metadata(place, allContributors.size()));
	...
	public Integer calculatePages(Integer count, Integer totalResults) {
		if (totalResults < count) {
	        log.debug("Desired count exceeds total result: count={}, result={}", count, totalResults);
